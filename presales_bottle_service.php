...
if(isset($_GET['status']) && isset($_GET['date'])){
    ...
    $bar = $_GET['bar'];
    if($status == 'saved'){
        $posts = selectAll('presales_bottle_service', ['DateReg' => $date, 'BarID' => $barId, 'Bar' => $bar ]);
        $total_profit = selectOne('presales_bottle_service', ['DateReg' => $date]);
...
<body>
                    ...
                    success: function(response) {
                        ...
                        if (response.includes('success')) {
                            Swal.fire({
                                ...
                                window.location =
                                    `presales_bottle_service.php?bar=${bar}&status=saved&date=${currentTime}`;
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
    ...
    function fetchdata() {
        ...
        if (date_input != "") {
            var bar = $('#bar').val()
            window.location =
                `presales_bottle_service.php?bar=${bar}&status=saved&date=${date_input}`;
    ...
    function refreshdata() {
        var bar = $('#bar').val()
        window.location =
            `presales_bottle_service.php?bar=${bar}`;
    }
    </script>
